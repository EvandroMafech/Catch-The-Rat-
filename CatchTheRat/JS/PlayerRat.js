...
import { rat_run_path_image,
         canvas } from "./constants.js"
...
class PlayerRat{
    constructor(x,y,size,imageElement) {
        ...
        this.height = 100 //usado para colisão
        this.width = 100   //usado para colisão   
    ...
    checkFloor(){
        ...
        obstacles.forEach(platform => {
             ...
             const rightEdge = canvas.offsetLeft + platform.position.x + platform.width - this.offsetImage
             const leftEdge = platform.position.x + canvas.offsetLeft - (this.width - this.offsetImage) 
            ...
            if (this.position.y > topEdge &&
                ...
                this.velocityY > 0  && 
                this.position.y + this.height - this.velocityY <= platform.position.y+canvas.offsetTop 
                ) {                  
    ...
    initPosition(image){ // inicia a posição dos players
        image.style.left = `${this.position.x}px`
        image.style.top = `${this.position.y}px`
    }

    size(size,imageElement){//atualiza o tamanho dos players
    ...
    moveLeft() {
        ...
        if(this.moving == false){
            this.image.src = rat_run_path_image
            this.moving = true
        }
    }
...
export default PlayerRat  //exporta a classe para onde o new é chamado